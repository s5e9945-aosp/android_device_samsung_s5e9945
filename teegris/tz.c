...
void iwd_connect(int fd) {
    const struct tz_uiwsock_connection conn = {.name = "root_task"};

    ioctl(fd, TZIO_UIWSOCK_CONNECT, conn);
    ioctl(fd, TZIO_UIWSOCK_WAIT_CONNECTION);
}

void iwd_send(int fd, char* buffer, uint64_t size) {
...
void* loader(void* efd) {
    ...
    while (true) {
        ...
        iwd_recv(fd, buf, sizeof(buf));
        load((int*)buf + 31, buf + 8);
        buf[104] = 9;
        iwd_send(fd, buf + 104, 160);
    }
