...
class TeegrisGateKeeper {
  public:
    TeegrisGateKeeper() {
        ...
        msg_verify.buffer = &shmem_sz_cmd.ptr;
        msg_verify.enrolled_password_handle = &shmem_sz_pwd_handle.ptr;
        msg_verify.provided_password = &shmem_sz_pwd.ptr;
    ...
    void Verify(const VerifyRequest& request, VerifyResponse* response) {
        ...
        memcpy(shmem_sz_pwd_handle.ptr, request.password_handle.Data<uint8_t>(),
               msg_verify.enrolled_password_handle_length);
        memcpy(shmem_sz_pwd.ptr, request.provided_password.Data<uint8_t>(),
               msg_verify.provided_password_length);
  ...
  private:
    ...
    struct shared_mem shmem_sz_cmd = {.size = 0x438, .mode = 3};
    struct shared_mem shmem_sz_pwd_handle = {.size = 0x40, .mode = 1};
    struct shared_mem shmem_sz_pwd = {.size = 0x80, .mode = 1};
